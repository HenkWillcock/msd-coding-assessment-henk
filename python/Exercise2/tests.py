...
class TestBirthingUnit(unittest.TestCase):
    ...
    def test_get_people(self):
        bu = BirthingUnit()
        people = bu.get_people(5)
        self.assertEqual(len(people), 5)

    def test_get_people_accumulates(self):
        bu = BirthingUnit()
        bu.get_people(3)
        bu.get_people(2)
        self.assertEqual(len(bu._people), 5)

    def test_when_get_people_then_names_are_valid(self):
        bu = BirthingUnit()
        people = bu.get_people(20)
        for p in people:
            self.assertIn(p.name, {"Bob", "Betty"})

    def test_when_get_people_then_both_names_appear_in_sample(self):
        """
    ...
    def test_when_get_people_none_under_15(self):
        bu = BirthingUnit()
        people = bu.get_people(20)
    ...
    def test_given_mixed_people_when_get_bobs_false_then_returns_all_bobs(self):
        bu = BirthingUnit()
        bu._people = [
            People("Bob"),
            People("Betty"),
    ...
    # This behaviour is wrong, the older_than_30 flag actually gives Bobs under 30.
    # But I've written a test for the current behaviour since this exercise is about
    # refactoring not changing behaviour.
    def test_when_get_bobs_over_30_flag_then_returns_expected_count(self):
        bu = BirthingUnit()
        ...
        bobs_over_30 = bu._get_bobs(older_than_30=True)
